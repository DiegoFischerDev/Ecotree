      ...
      <div className='rounded border mt-10'>
        ...
        <div className='flex'>
          ...
          <div className='mx-5 py-2 text-xl'>
            ...
            {ListaDeOrçamentos.map((orçamento) => <input type="text" onChange={(e) => { setItemModificado(orçamento.id) }} key={orçamento.id} placeholder={orçamento.cliente} className={`w-[180px] h-10 mt-3 flex items-center  ${orçamento.id === itemModificado ? "text-green-400 placeholder-green-400" : "text-gray-400 placeholder-gray-400"}`}></input>)}
          </div>

          <div className='mx-5 py-2 text-xl'>
            <h4 className="h-10 flex items-center text-gray-600">Categoria</h4>
            {ListaDeOrçamentos.map((orçamento) => <select key={orçamento.id} onChange={() => { setItemModificado(orçamento.id) }} className={`w-[130px] h-10 mt-3 flex items-center  ${orçamento.id === itemModificado ? "text-green-400" : "text-gray-400"}`}>
          ...
          <div className='mx-5 py-2 text-xl'>
            ...
            {ListaDeOrçamentos.map((orçamento) => <input key={orçamento.id} type="date" onChange={() => { setItemModificado(orçamento.id) }} className={`w-[160px] h-10 mt-3 flex items-center  ${orçamento.id === itemModificado ? "text-green-400" : "text-gray-400"}`}></input>)}
          </div>
          ...
          <div className='mx-5 py-2 text-xl'>
            ...
            {ListaDeOrçamentos.map((orçamento) => <button onClick={() => { setItemModificado(0) }} key={orçamento.id} className={`w-[100px] ${orçamento.id === itemModificado ? "bg-green-400" : "bg-gray-400"} h-10 mt-3 flex items-center justify-center text-white rounded`}>Salvar</button>)}
          </div>
